"""
Repository Test Suite for Integrated Payroll Engine (payroll).
"""
import pytest
import asyncio
from backend.payroll.domain.entities import PayrollEntity
from backend.payroll.infrastructure.repositories import InMemoryPayrollRepository

def test_payroll_repository_crud():
    async def _run():
        repo = InMemoryPayrollRepository()
        
        # Save entity
        entity = PayrollEntity(code="R-TEST", name="Repository Test Integrated Payroll Engine", status="ACTIVE")
        saved = await repo.save(entity)
        assert saved.id is not None

        # Get entity
        fetched = await repo.get_by_id(saved.id)
        assert fetched is not None
        assert fetched.code == "R-TEST"

        # List entities
        items = await repo.list_all()
        assert len(items) >= 1

        # Delete entity
        deleted = await repo.delete(saved.id)
        assert deleted is True

    asyncio.run(_run())
