"""
Repository Test Suite for Faculty & Workload Management (faculty).
"""
import pytest
import asyncio
from backend.faculty.domain.entities import FacultyEntity
from backend.faculty.infrastructure.repositories import InMemoryFacultyRepository

def test_faculty_repository_crud():
    async def _run():
        repo = InMemoryFacultyRepository()
        
        # Save entity
        entity = FacultyEntity(code="R-TEST", name="Repository Test Faculty & Workload Management", status="ACTIVE")
        saved = await repo.save(entity)
        assert saved.id is not None

        # Get entity
        fetched = await repo.get_by_id(saved.id)
        assert fetched is not None
        assert fetched.code == "R-TEST"

        # List entities
        items = await repo.list_all()
        assert len(items) >= 1

        # Delete entity
        deleted = await repo.delete(saved.id)
        assert deleted is True

    asyncio.run(_run())
