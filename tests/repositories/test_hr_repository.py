"""
Repository Test Suite for Human Resource & Recruitment (hr).
"""
import pytest
import asyncio
from backend.hr.domain.entities import HrEntity
from backend.hr.infrastructure.repositories import InMemoryHrRepository

def test_hr_repository_crud():
    async def _run():
        repo = InMemoryHrRepository()
        
        # Save entity
        entity = HrEntity(code="R-TEST", name="Repository Test Human Resource & Recruitment", status="ACTIVE")
        saved = await repo.save(entity)
        assert saved.id is not None

        # Get entity
        fetched = await repo.get_by_id(saved.id)
        assert fetched is not None
        assert fetched.code == "R-TEST"

        # List entities
        items = await repo.list_all()
        assert len(items) >= 1

        # Delete entity
        deleted = await repo.delete(saved.id)
        assert deleted is True

    asyncio.run(_run())
