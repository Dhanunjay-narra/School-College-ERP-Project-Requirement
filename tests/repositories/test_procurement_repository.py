"""
Repository Test Suite for Procurement Management (procurement).
"""
import pytest
import asyncio
from backend.procurement.domain.entities import ProcurementEntity
from backend.procurement.infrastructure.repositories import InMemoryProcurementRepository

def test_procurement_repository_crud():
    async def _run():
        repo = InMemoryProcurementRepository()
        
        # Save entity
        entity = ProcurementEntity(code="R-TEST", name="Repository Test Procurement Management", status="ACTIVE")
        saved = await repo.save(entity)
        assert saved.id is not None

        # Get entity
        fetched = await repo.get_by_id(saved.id)
        assert fetched is not None
        assert fetched.code == "R-TEST"

        # List entities
        items = await repo.list_all()
        assert len(items) >= 1

        # Delete entity
        deleted = await repo.delete(saved.id)
        assert deleted is True

    asyncio.run(_run())
