"""
Repository Test Suite for BI & Institutional Analytics (analytics).
"""
import pytest
import asyncio
from backend.analytics.domain.entities import AnalyticsEntity
from backend.analytics.infrastructure.repositories import InMemoryAnalyticsRepository

def test_analytics_repository_crud():
    async def _run():
        repo = InMemoryAnalyticsRepository()
        
        # Save entity
        entity = AnalyticsEntity(code="R-TEST", name="Repository Test BI & Institutional Analytics", status="ACTIVE")
        saved = await repo.save(entity)
        assert saved.id is not None

        # Get entity
        fetched = await repo.get_by_id(saved.id)
        assert fetched is not None
        assert fetched.code == "R-TEST"

        # List entities
        items = await repo.list_all()
        assert len(items) >= 1

        # Delete entity
        deleted = await repo.delete(saved.id)
        assert deleted is True

    asyncio.run(_run())
