"""
Repository Test Suite for Multi-Store Warehouse Management (warehouses).
"""
import pytest
import asyncio
from backend.warehouses.domain.entities import WarehousesEntity
from backend.warehouses.infrastructure.repositories import InMemoryWarehousesRepository

def test_warehouses_repository_crud():
    async def _run():
        repo = InMemoryWarehousesRepository()
        
        # Save entity
        entity = WarehousesEntity(code="R-TEST", name="Repository Test Multi-Store Warehouse Management", status="ACTIVE")
        saved = await repo.save(entity)
        assert saved.id is not None

        # Get entity
        fetched = await repo.get_by_id(saved.id)
        assert fetched is not None
        assert fetched.code == "R-TEST"

        # List entities
        items = await repo.list_all()
        assert len(items) >= 1

        # Delete entity
        deleted = await repo.delete(saved.id)
        assert deleted is True

    asyncio.run(_run())
