"""
Institutional CRM & Admissions Leads — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for crm.
"""
import json
import csv
import io
...
class CrmSerializer:
    @staticmethod
    def to_json(entity: CrmEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[CrmEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: CrmEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
