"""
Centralized Faceted Search — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for search.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.search.metrics")

class SearchMetricsCollector:
    """Domain telemetry collector for Centralized Faceted Search."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: search.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
