"""
Examinations & Grading — Concrete Repository Implementation.
"""
...
class InMemoryExaminationsRepository(IExaminationsRepository):
    def __init__(self):
        self._items: Dict[str, ExaminationsEntity] = {}
        self._seed_sample_records()

    def _seed_sample_records(self):
        sample = ExaminationsEntity(
            id=f"EXAMINATIONS-001",
            code="SAMPLE-01",
            name="Primary Standard Examinations & Grading Record",
            status="ACTIVE",
            metadata={"description": "Exam schedules, hall allocation, question banks, marks moderation, CGPA transcripts", "priority": "HIGH"}
        )
...
default_examinations_repo = InMemoryExaminationsRepository()
