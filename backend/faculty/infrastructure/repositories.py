"""
Faculty & Workload Management — Concrete Repository Implementation.
"""
...
class InMemoryFacultyRepository(IFacultyRepository):
    def __init__(self):
        self._items: Dict[str, FacultyEntity] = {}
        self._seed_sample_records()

    def _seed_sample_records(self):
        sample = FacultyEntity(
            id=f"FACULTY-001",
            code="SAMPLE-01",
            name="Primary Standard Faculty & Workload Management Record",
            status="ACTIVE",
            metadata={"description": "Faculty profiles, workload balancing, research, teaching assignments", "priority": "HIGH"}
        )
...
default_faculty_repo = InMemoryFacultyRepository()
