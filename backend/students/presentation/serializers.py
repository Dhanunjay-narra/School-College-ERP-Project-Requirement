"""
Student Information & Lifecycle — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for students.
"""
import json
import csv
import io
...
class StudentsSerializer:
    @staticmethod
    def to_json(entity: StudentsEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[StudentsEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: StudentsEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
