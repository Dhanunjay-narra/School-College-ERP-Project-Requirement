"""
Human Resource & Recruitment — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for hr.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.hr.metrics")

class HrMetricsCollector:
    """Domain telemetry collector for Human Resource & Recruitment."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: hr.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
