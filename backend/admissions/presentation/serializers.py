"""
Admissions CRM & Merit Engine — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for admissions.
"""
import json
import csv
import io
...
class AdmissionsSerializer:
    @staticmethod
    def to_json(entity: AdmissionsEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[AdmissionsEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: AdmissionsEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
