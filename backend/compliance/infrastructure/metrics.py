"""
Accreditation & Regulatory Compliance — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for compliance.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.compliance.metrics")

class ComplianceMetricsCollector:
    """Domain telemetry collector for Accreditation & Regulatory Compliance."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: compliance.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
