"""
Universal Multi-Channel Notifications — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for communication.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.communication.metrics")

class CommunicationMetricsCollector:
    """Domain telemetry collector for Universal Multi-Channel Notifications."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: communication.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
