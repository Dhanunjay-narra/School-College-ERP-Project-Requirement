"""
Hostel & Housing Management — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for hostels.
"""
import json
import csv
import io
...
class HostelsSerializer:
    @staticmethod
    def to_json(entity: HostelsEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[HostelsEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: HostelsEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
