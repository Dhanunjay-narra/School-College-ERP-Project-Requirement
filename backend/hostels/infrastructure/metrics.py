"""
Hostel & Housing Management — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for hostels.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.hostels.metrics")

class HostelsMetricsCollector:
    """Domain telemetry collector for Hostel & Housing Management."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: hostels.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
