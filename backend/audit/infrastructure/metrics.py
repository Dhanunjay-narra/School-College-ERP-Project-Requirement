"""
Immutable Audit Logging — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for audit.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.audit.metrics")

class AuditMetricsCollector:
    """Domain telemetry collector for Immutable Audit Logging."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: audit.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
