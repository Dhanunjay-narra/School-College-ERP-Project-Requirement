"""
Integrated Payroll Engine — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for payroll.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.payroll.metrics")

class PayrollMetricsCollector:
    """Domain telemetry collector for Integrated Payroll Engine."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: payroll.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
