"""
Integrated Payroll Engine — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for payroll.
"""
import json
import csv
import io
...
class PayrollSerializer:
    @staticmethod
    def to_json(entity: PayrollEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[PayrollEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: PayrollEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
