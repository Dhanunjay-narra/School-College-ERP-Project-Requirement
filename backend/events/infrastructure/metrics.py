"""
Campus Events & Conferences — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for events.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.events.metrics")

class EventsMetricsCollector:
    """Domain telemetry collector for Campus Events & Conferences."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: events.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
