"""
Campus Events & Conferences — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for events.
"""
import json
import csv
import io
...
class EventsSerializer:
    @staticmethod
    def to_json(entity: EventsEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[EventsEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: EventsEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
