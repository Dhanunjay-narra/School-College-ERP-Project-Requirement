"""
Library & RFID Circulation — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for library.
"""
import json
import csv
import io
...
class LibrarySerializer:
    @staticmethod
    def to_json(entity: LibraryEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[LibraryEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: LibraryEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
