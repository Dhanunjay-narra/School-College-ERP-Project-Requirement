"""
Library & RFID Circulation — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for library.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.library.metrics")

class LibraryMetricsCollector:
    """Domain telemetry collector for Library & RFID Circulation."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: library.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
