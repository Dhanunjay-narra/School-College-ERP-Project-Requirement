"""
Finance & General Ledger — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for finance.
"""
import json
import csv
import io
...
class FinanceSerializer:
    @staticmethod
    def to_json(entity: FinanceEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[FinanceEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: FinanceEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
