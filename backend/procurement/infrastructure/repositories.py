"""
Procurement Management — Concrete Repository Implementation.
"""
...
class InMemoryProcurementRepository(IProcurementRepository):
    def __init__(self):
        self._items: Dict[str, ProcurementEntity] = {}
        self._seed_sample_records()

    def _seed_sample_records(self):
        sample = ProcurementEntity(
            id=f"PROCUREMENT-001",
            code="SAMPLE-01",
            name="Primary Standard Procurement Management Record",
            status="ACTIVE",
            metadata={"description": "Requisitions, RFQs, vendor quotations, scoring, POs, 3-way invoice matching", "priority": "HIGH"}
        )
...
default_procurement_repo = InMemoryProcurementRepository()
