"""
Procurement Management — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for procurement.
"""
import json
import csv
import io
...
class ProcurementSerializer:
    @staticmethod
    def to_json(entity: ProcurementEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[ProcurementEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: ProcurementEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
