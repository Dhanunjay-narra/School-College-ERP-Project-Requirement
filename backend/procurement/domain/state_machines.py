"""
Procurement Management — Finite State Machine (FSM) Transition Engine.
"""
from typing import Set, Dict, List
from backend.core.exceptions import DomainException

class ProcurementStateMachine:
    """State machine coordinator for Procurement Management."""
    ...
    def transition(cls, current_state: str, new_state: str) -> str:
        if not cls.can_transition(current_state, new_state):
            raise DomainException(f"State transition from '{current_state}' to '{new_state}' is disallowed in procurement.")
