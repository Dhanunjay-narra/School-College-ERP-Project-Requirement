"""
Campus Inventory & Stores — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for inventory.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.inventory.metrics")

class InventoryMetricsCollector:
    """Domain telemetry collector for Campus Inventory & Stores."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: inventory.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
