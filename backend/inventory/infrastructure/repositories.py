"""
Campus Inventory & Stores — Concrete Repository Implementation.
"""
...
class InMemoryInventoryRepository(IInventoryRepository):
    def __init__(self):
        self._items: Dict[str, InventoryEntity] = {}
        self._seed_sample_records()

    def _seed_sample_records(self):
        sample = InventoryEntity(
            id=f"INVENTORY-001",
            code="SAMPLE-01",
            name="Primary Standard Campus Inventory & Stores Record",
            status="ACTIVE",
            metadata={"description": "Central/Science/Sports stores, SKUs, reorder levels, stock movements", "priority": "HIGH"}
        )
...
default_inventory_repo = InMemoryInventoryRepository()
