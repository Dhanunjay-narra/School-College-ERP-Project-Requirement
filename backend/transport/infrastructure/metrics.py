"""
Transportation & GPS Fleet — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for transport.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.transport.metrics")

class TransportMetricsCollector:
    """Domain telemetry collector for Transportation & GPS Fleet."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: transport.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
