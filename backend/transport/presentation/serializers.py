"""
Transportation & GPS Fleet — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for transport.
"""
import json
import csv
import io
...
class TransportSerializer:
    @staticmethod
    def to_json(entity: TransportEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[TransportEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: TransportEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
