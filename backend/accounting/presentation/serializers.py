"""
Accounts Payable & Receivable — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for accounting.
"""
import json
import csv
import io
...
class AccountingSerializer:
    @staticmethod
    def to_json(entity: AccountingEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[AccountingEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: AccountingEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
