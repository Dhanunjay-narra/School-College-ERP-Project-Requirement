"""
Alumni Network & Relations — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for alumni.
"""
import json
import csv
import io
...
class AlumniSerializer:
    @staticmethod
    def to_json(entity: AlumniEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[AlumniEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: AlumniEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
