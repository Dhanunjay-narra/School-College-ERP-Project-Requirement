"""
Asset Lifecycle & Depreciation — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for assets.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.assets.metrics")

class AssetsMetricsCollector:
    """Domain telemetry collector for Asset Lifecycle & Depreciation."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: assets.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
