"""
Vendor Management & Compliance — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for vendors.
"""
import json
import csv
import io
...
class VendorsSerializer:
    @staticmethod
    def to_json(entity: VendorsEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[VendorsEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: VendorsEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
