"""
Multi-Store Warehouse Management — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for warehouses.
"""
import json
import csv
import io
...
class WarehousesSerializer:
    @staticmethod
    def to_json(entity: WarehousesEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[WarehousesEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: WarehousesEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
