"""
Parent & Guardian Management — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for parents.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.parents.metrics")

class ParentsMetricsCollector:
    """Domain telemetry collector for Parent & Guardian Management."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: parents.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
