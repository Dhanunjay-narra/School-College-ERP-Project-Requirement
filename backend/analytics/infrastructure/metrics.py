"""
BI & Institutional Analytics — Prometheus & OpenTelemetry Instrumentation.
Collects latency histograms, request counters, and error rates for analytics.
"""
import time
import logging
from typing import Dict, Any

logger = logging.getLogger("erp.analytics.metrics")

class AnalyticsMetricsCollector:
    """Domain telemetry collector for BI & Institutional Analytics."""
    ...
    def record_request(cls, endpoint: str, latency_ms: float, is_error: bool = False):
        ...
        if is_error:
            cls._error_count += 1
        logger.debug(f"Metric logged: analytics.{endpoint} -> {latency_ms:.2f}ms (Errors: {cls._error_count})")
