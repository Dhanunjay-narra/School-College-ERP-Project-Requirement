"""
Fees & Student Billing — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for fees.
"""
import json
import csv
import io
...
class FeesSerializer:
    @staticmethod
    def to_json(entity: FeesEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[FeesEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: FeesEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
