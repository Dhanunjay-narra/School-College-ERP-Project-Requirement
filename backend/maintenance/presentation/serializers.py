"""
Campus Facility Maintenance — Serializers, Formatter & Exporters.
Provides CSV, JSON, and XML serialization routines for maintenance.
"""
import json
import csv
import io
...
class MaintenanceSerializer:
    @staticmethod
    def to_json(entity: MaintenanceEntity) -> str:
        return json.dumps(entity.to_dict(), indent=2)

    @staticmethod
    def to_csv(entities: List[MaintenanceEntity]) -> str:
        output = io.StringIO()
    ...
    @staticmethod
    def to_summary(entity: MaintenanceEntity) -> Dict[str, Any]:
        return {
            "id": entity.id,
