...
def create_all_prs():
    ...
    # 1. Create a true bare initial root commit on main
    print("\n--- [STEP 0] Resetting main branch on GitHub to clean initial commit ---", flush=True)
    run_git("git checkout --orphan bare_root")
    ...
    run_git("git push origin bare_root:main --force")
    time.sleep(2)
    ...
    pr_steps = [
        (
            ...
            "Implements Phase 01 to Phase 03: core architecture, event broker, authentication, JWT tokens, PBKDF2 cryptography, RBAC, and multi-campus hierarchy.",
        ...
        (
            ...
            "Implements Phase 04 to Phase 11: 8-stage student lifecycle, parent portal, admissions merit ranking, timetable conflict detection, attendance engine, examinations, and LMS assignments.",
        ...
        (
            ...
            "Implements Phase 12 to 16 and 28: fee invoice scheduling, payment gateway abstraction, general ledger double-entry journal, procurement RFQ, warehouse inventory, and campus store POS.",
        ...
        (
            ...
            "Implements Phase 17 to 21 and 26 to 27: HR employee lifecycle, payroll tax deductions, bus GPS route tracking, hostel room allocations, MARC21 RFID library circulation, and research grants.",
        ...
        (
            ...
            "Implements Phase 22 to 25, 29 to 30, and 38 to 45: CRM alumni relations, multi-channel notifications, document vault signatures, configurable workflows, machine learning models, and main FastAPI ASGI application.",
        ...
        (
            ...
            "Implements Phase 31 to 37: modern React + Tailwind web SPA with 1-click persona logins, Flutter mobile application, 238+ passing unit/integration tests, Docker Compose, Kubernetes manifests, Helm charts, and Terraform AWS IaC.",
            "frontend/ mobile/ database/ infrastructure/ docs/ tests/ docker-compose.yml .github/ scripts/"
        )
    ]

    for step_num, branch_name, pr_title, pr_body, paths in pr_steps:
        print(f"\n==================== [PR #{step_num}] {branch_name} ====================", flush=True)
        
        # 1. Fetch latest main from origin
        run_git("git fetch origin main")
        
        # 2. Checkout fresh branch off origin/main
        run_git(f"git checkout -B {branch_name} origin/main")
        
        # 3. Add and commit files
        run_git(f"git add {paths}")
        ...
        print(f"[GITHUB API] Creating Pull Request for {branch_name}...", flush=True)
        pr_payload = {
            "title": pr_title,
            "body": pr_body,
            "head": branch_name,
        ...
        print(f"[GITHUB API] PR #{pr_num} created: {pr_resp.get('html_url')}", flush=True)
        time.sleep(2)
        
        # 6. Merge Pull Request on GitHub
        print(f"[GITHUB API] Merging PR #{pr_num} on GitHub...", flush=True)
        merge_payload = {
            "commit_title": f"Merge pull request #{pr_num} from {REPO_OWNER}/{branch_name}",
            "commit_message": pr_body,
            "merge_method": "merge"
        }
        merge_resp = github_req(f"pulls/{pr_num}/merge", method="PUT", data=merge_payload, token=token)
        print(f"[GITHUB API] PR #{pr_num} Merged: {merge_resp.get('merged')}", flush=True)
        time.sleep(2)

    # 7. Sync local main
    print("\n--- [FINAL] Updating local main with all 6 merged PRs ---", flush=True)
    run_git("git checkout -B main origin/main")
    run_git("git pull origin main")
    print("\n[SUCCESS] All 6 Pull Requests created and merged on GitHub!", flush=True)
