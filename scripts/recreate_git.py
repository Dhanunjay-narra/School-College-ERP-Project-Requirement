...
def recreate():
    print("[GIT REBUILD] Creating clean git history with 6 feature branches and 6 merged PRs...")
    
    # Create orphan branch
    run("git checkout --orphan clean_root")
    run("git rm -rf --cached .")
    
    # 0. Initial commit
    run("git add README.md LICENSE .gitignore .env.example requirements.txt pyproject.toml Makefile")
    run("git commit -m \"chore: initial enterprise repository structure\"")
    run("git branch -M main")
    
    # PR 1: feat/foundation-identity-organization
    ...
    # PR 2: feat/education-core-academic-engine
    ...
    # PR 3: feat/finance-procurement-supply-chain
    ...
    # PR 4: feat/campus-operations-hr-infrastructure
    ...
    # PR 5: feat/enterprise-platform-ai-analytics
    ...
    # PR 6: feat/frontend-portals-devops-testing
    run("git checkout -b feat/frontend-portals-devops-testing")
    run("git add frontend/ mobile/ database/ infrastructure/ docs/ tests/ docker-compose.yml pytest.ini scripts/")
    ...
    print("[GIT REBUILD] Complete! Verification log:")
    subprocess.run("git log --graph --oneline -n 30", shell=True, cwd=BASE)
